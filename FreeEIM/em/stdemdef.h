...
public:
	// 用户信息数据包初始化
	EM_USERINFO::EM_USERINFO(TCHAR *szhost, TCHAR *szip, TCHAR *szGroup)
	{
		ASSERT(NULL != szhost);
		ASSERT(NULL != szip);
		ASSERT(NULL != szGroup);
...
public:
	...
	void GetData(char **data, int *len)
	{
		...
		*data = new TCHAR[*len];
		...
		CString str;
		str.Format(TEXT("%d"), m_nDateLen);
		MessageBox(0,str,str,0);
		s_nOffset = sizeof(int)*2+m_nDateLen;
		memcpy(*data, m_buf+s_nOffset, *len);
	}

	void SetData(TCHAR *data, int len)
	{
		ASSERT(NULL != data);
		ASSERT(0 < len);

		m_bData = new TCHAR[len];
		memcpy(m_bData,data,len);
		m_nDataLen = len;
	}

	void SetDate(TCHAR *date, int len)
	{
		ASSERT(NULL != date);
		ASSERT(0 < len);
		
		m_bDate = new TCHAR[len];
